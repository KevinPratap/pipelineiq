...
export async function POST() {
  try {
    ...
    for (const deal of deals) {
      await prisma.deal.create({ data: { ...deal, userId: user.id, pipelineId: pipeline.id } })
    }

    return NextResponse.json({ success: true, message: "Database seeded", credentials: { email: "demo@pipelineiq.dev", password: "demo1234" } })
  } catch (error) {
